...
local conf = {
    ...
    itemSellerId = 9611, -- register the item
    exhaust = 120,
...
function getTimeInWords(seconds)
    ...
    if minutes > 0 then
        timeWords = minutes .. " minute(s)"
    end

    if remainingSeconds > 0 then
        timeWords = timeWords .. (minutes > 0 and " and " or "") .. remainingSeconds .. " second(s)"
    end
...
function itemSeller.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if player:getExhaustion("itemSellerExhaustion") > 0 then
        player:sendCancelMessage(
            "You must wait " ..
                getTimeInWords(player:getExhaustion("itemSellerExhaustion")) ..
                    " to use this item again."
        )
        return true
    end
    ...
    if lootPouch then
        normalPouch = lootPouch:getItemHoldingCount()
    end
    if lootPouchStore then
        storePouch = lootPouchStore:getItemHoldingCount()
    end
    ...
    player:sendTextMessage(
        ...
        "You sold " ..
            totalSelled ..
                " items and received " ..
                    totalEarn .. " golds, which were credited to your bank account."
    )
    if conf.toggleLogger then
        logger.info(
            player:getName() ..
                ":: used itemSeller and sold " ..
                    totalSelled .. " items, receiving " .. totalEarn .. " golds."
        )
    end
    return true
end
