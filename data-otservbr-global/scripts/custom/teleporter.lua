...
function supremeCube.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    -- AQUI CHECA SE TOMOU BAN HUNT
    local banExpirationTime = player:kv():get("exp-ban")
    if banExpirationTime and banExpirationTime > os.time() then
        supremeCubeMessage(player, CONST_ME_POFF, "You can only use teleport cube after your exp punishment expires.")
        return false
    end

    -- Verificar se o jogador está em luta
    if inFight then
        supremeCubeMessage(player, CONST_ME_POFF, "You cant use this while in battle.")
        return false
    end

    -- Verificar se o jogador tem dinheiro suficiente
    if player:getMoney() + player:getBankBalance() < config.price then
        supremeCubeMessage(player, CONST_ME_POFF, "You have no money.")
        return false
    end

    -- Verificar se o tempo de espera (cooldown) expirou
    if player:getStorageValue(config.storage) > os.time() then
        local remainingTime = player:getStorageValue(config.storage) - os.time()
        supremeCubeMessage(player, CONST_ME_POFF, "You can use again in: " .. remainingTime .. " seconds.")
        return false
    end
    ...
    if vocationId == VOCATION_BASE_ID.NONE then
        local window = ModalWindow({
            title = "Supreme Cube",
            message = "Selecione um destino - Preço: " .. config.price .. " gold.",
        })
    ...
    elseif vocationId == VOCATION_BASE_ID.SORCERER or vocationId == VOCATION_BASE_ID.DRUID or vocationId == VOCATION_BASE_ID.PALADIN or vocationId == VOCATION_BASE_ID.KNIGHT or vocationId == VOCATION_BASE_ID.MASTER_SORCERER or vocationId == VOCATION_BASE_ID.ELDER_DRUID or vocationId == VOCATION_BASE_ID.ROYAL_PALADIN or vocationId == VOCATION_BASE_ID.ELITE_KNIGHT then
        ...
        window:addChoice("House", function(player, button, choice)
            if button.name == "Select" then
                ...
                if house then
                    ...
                    supremeCubeMessage(player, CONST_ME_POFF, "You dont own a house.")
                end
            end
            return true
        end)
            ...
            if town.name then
                window:addChoice(town.name, function(player, button, choice)
                    if button.name == "Select" then
                        ...
                        player:removeMoneyBank(config.price)
                        supremeCubeMessage(player, CONST_ME_TELEPORT, "You were teleported to " .. town.name .. ".")
                        player:setStorageValue(config.storage, os.time() + config.cooldown)
                    end
                    return true
                end)
        ...
        supremeCubeMessage(player, CONST_ME_POFF, "You can only use this with a vocation.")
    end
